...
encodings_provided(Req, Context) ->
    entity_resource:encodings_provided(?MODULE, Req, Context).

file_path(Context, "") ->
    filename:join([Context#context.root, "index.html"]);
...
content_types_provided(ReqData, Context) ->
    FP = file_path(Context, wrq:disp_path(ReqData)),
    CT = webmachine_util:guess_mime(FP),
...
generate_etag(ReqData, Context) ->
    case maybe_fetch_object(Context, wrq:disp_path(ReqData)) of
        {true, BodyContext} ->
            ETag = util:sha(BodyContext#context.response_body),
            {ETag, ReqData,
