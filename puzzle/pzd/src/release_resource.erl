...
%
% @doc Resource module for release.
%
% $Id$
%

-module(release_resource).
...
process_post(Req, Context) ->
    ...
    {Rel} = entity:unmochify(Moch),
    logger:debug("~p:process_post() Rel: ~p",[?ENTITY, Rel]),

    Key = release:save(Rel),
    {ok, Release} = release:select(Key),

    Json = entity:to_json(Release),

    Req1 = wrq:set_resp_header("Content-Type", "application/json",Req),
    Req2 = wrq:append_to_resp_body(Json, Req1),

    {true,Req2,Context#context{value=Release}}.
