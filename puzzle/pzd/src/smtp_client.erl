...
%
% @doc SMTP client adapted from original source:
%%%---------------------------------------------------------------------
...
-define(elog(X,Y), logger:info("~p:~p: " X,
                               [?MODULE, ?LINE | Y])).
...
deliver(From, To, Subject, Mail, Opts) ->
    Self = self(),
    Pid = spawn(fun() -> init(Self,From,To,Subject,Mail,Opts) end),
...
init(Pid,From,To,Subject,Mail,Opts) ->
    Answer = case catch doit(From,To,Subject,Mail,Opts) of
...
doit(From,To,Subject,Mail,Opts) ->
    S = init_session(From,To,Opts),
    connect(S, crlf(lists:flatten(format(To,From,Subject,Mail)))).

format(To, From, Subject, Mail) ->
    M1 = lists:append("To: "++To++"\n",Mail),
    M2 = lists:append("From: "++From++"\n",M1),
    lists:append("Subject: "++Subject++"\n",M2).
