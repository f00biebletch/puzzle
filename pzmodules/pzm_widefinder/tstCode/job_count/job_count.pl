...
my %ENV = (
    ...
    NAME => "job_count",
    TITLE => "count log file occurrances",
    DETAIL => "count log file occurances and sort by count",
...
sub generate_tpt{
    ...
    my %ans = ();
    my $err = 0;
    my $tstabspath = $root."/".$ENV{TSTRELPATH}."/";
    my $ipath = $tstabspath."../input/";

    my @inputVector = @{$ENV{INPUT}{DATA}{INPUT}{FILE}};

    
    ##start
        # get the info of this job
        my $infoCmd = $root."/tpt/tpt_count.pl -a info";
        ...
        if(exists $tmp->{STATUS}){
            if(lc($tmp->{STATUS}) eq (lc("0"))){
                ...
                if(lc($tmp->{VALUE}{NAME} eq "tpt_count")) {
                    ...
                    for my $ilogfile ( @inputVector){

                        $count+=1;

                        $string  = 
                            sprintf("$root/tpt/tpt_count\_%08d.pl",
                                    $count);
                        $tptname = sprintf("tpt_count\_%08d",
                                           $count);
                        ...
                        $infoTPT->{INPUT}{DATA}{INPUT}{DIR}= $ipath;
                        $infoTPT->{INPUT}{DATA}{INPUT}{FILE}= $ilogfile;
                        
                        #specify results log for count output
                        $infoTPT->{OUTPUT}{DIR}=$tptname;
                        $infoTPT->{OUTPUT}{DATA}{RESULTS_LOG}{FILE}=$ilogfile.".out";
                        #
                        
                        ### CREATE TPT real file.
                        setConfigTPT( $root."/tpt/tpt_count.pl",
                                      $infoTPT,
                                      $root."/tpt/".$tptname.".pl"
                            );
