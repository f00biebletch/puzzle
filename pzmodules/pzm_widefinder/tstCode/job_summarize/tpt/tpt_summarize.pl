...
#
# http://wiki.eng-scl.dolby.net:88/doku.php?id=deviut:tools:jurassikpark:modules
#
###############################################################################
...
#        JOB_INFO data. See wiki.eng-scl.dolby.net for details.
#
# {{{ Environment Section
#<HEADER><BEGIN>
#<ENV><BEGIN>
my %ENV = (
    ...
    NAME => "tpt_summarize",
    TITLE => "plot json for counts",
    DETAIL => "json results for counts",
...
sub do_run{
    ...
    my $logMsg = "Summarize , START , ".getDateTimeNow();
    logFileDump($tmp,$logMsg);
    #

#open the output file to write to and the input file to read from
#open the input file and process each line. Lines starting with URI
#indicate a new section - save it to temp, use it to push keys on
    open(JLOG, ">$ENV{OUTPUT}{PATH}"."/".$INFO{OUTPUT}{DATA}{COUNT}{FILE}) or die "cannot open summary outfile, $!\n"; 
    open(ILOG, "<$INFO{INPUT}{DATA}{COUNT}{DIR}"."/".$INFO{INPUT}{DATA}{COUNT}{FILE}) or die "cannot open count output, $!\n";
    while (<ILOG>) {
        if ($_ =~ /^URI/) {
            $tmp = $_;
            }
        else {
          push(@{$jhash{$tmp}{key}}, $_);
        }
    }
    close(ILOG);
    my $jStr = to_json(\%jhash);
    print JLOG $jStr;
    close(JLOG);
